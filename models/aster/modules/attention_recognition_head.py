...
class DecoderUnit(nn.Module):
    ...
    def forward(self, x, sPrev, yPrev):
        ...
        self.attention_unit = self.attention_unit.to(device)
        alpha = self.attention_unit(x.to(device), sPrev.to(device))
        context = torch.bmm(alpha.unsqueeze(1), x).squeeze(1)
        yProj = self.tgt_embedding(yPrev.long().to(device))
        # self.gru.flatten_parameters()
        self.gru = self.gru.to(device)
        output, state = self.gru(torch.cat([yProj, context.to(device)], 1).unsqueeze(1), sPrev.to(device))
